...
set "DATA_DIR=%LOCALAPPDATA%\YiboVibe_Sim_2"
set "CARGO_TARGET_DIR=F:\Download\GitHub\YiboVibe\target-sim2"
set "NODE_EXE="
if not "%NVM_SYMLINK%"=="" set "NODE_EXE=%NVM_SYMLINK%\node.exe"
if "%NODE_EXE%"=="" if exist "D:\Program\nodejs\node.exe" set "NODE_EXE=D:\Program\nodejs\node.exe"
set "TAURI_CLI_JS=F:\Download\GitHub\YiboVibe\desktop\node_modules\@tauri-apps\cli\tauri.js"
...
echo ----------------------------------------------------
echo 正在启动 YiboVibe 模拟实例: %SIM_NAME%
echo 隔离数据路径: %DATA_DIR%
echo 隔离编译路径: %CARGO_TARGET_DIR%
echo 独立 Dev 端口: 1421
echo 独立应用标识: com.yibovibe.desktop.sim2
echo ----------------------------------------------------

set "YIBOVIBE_DATA_DIR=%DATA_DIR%"
set "YIBOVIBE_ALLOW_MULTI_INSTANCE=1"
set "YIBOVIBE_INSTANCE_TAG=sim2"
set "TAURI_HMR_PORT=1421"

cd /d "F:\Download\GitHub\YiboVibe\desktop"
